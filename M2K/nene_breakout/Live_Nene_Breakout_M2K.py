                ...
                if self.connected:
                        ...
                        if self.position == 0 and self.global_position == 0:             # if there's not opened positions
                            if new_bar and current_len >= kc_period+pattern_len-2:
                                ...
                                if not (self.weekday == 4 and pd.to_datetime(self.hour).time() > pd.to_datetime('12:00:00').time()):
                                    ## Buys
                                    if not sent and sum(recognition == pattern_long) == pattern_len and prices[-2] < upper_band[-2] and prices[-1] > upper_band[-1] and plus_di[-1] > 40 and plus_di[-1] > minus_di[-1]:
                    ...
                    if self.position != 0:
                        ...
                        if self.position > 0:
                            ...
                            # By stop ==========
                            ## Stop 1
                            if self.check_pendings(ord_buy_sl_1) and self.position > 0:   # Check if stop 1 is filled
                            ...
                            ### Stop 1
                            if not self.check_pendings(ord_buy_sl_1) and sl_buy_1 - self.data.iloc[-1].low >= 1 and not exit_1 and self.position > 0:
                                self.exit_market(ord_buy_tp_1, 'BUY', contracts/2, price_buy_in_1, time_buy_in, comm_buy_in_1, 'fsl1')
                                sent = False
                            ### Stop 2
                            if not self.check_pendings(ord_buy_sl_2) and sl_buy_2 - self.data.iloc[-1].low >= 1 and self.position > 0:
                        ...
                        if self.position < 0:
                            ...
                            # By stop ==========
                            ## Stop 1
                            if self.check_pendings(ord_sell_sl_1) and self.position < 0:   # Check if stop 1 is filled
                            ...
                            #if 6 <= datetime.now().hour <= 20:
                            ### Stop 1
                            if not self.check_pendings(ord_sell_sl_1) and self.data.iloc[-1].high - sl_sell_1 >= 1 and not exit_1 and self.position < 0:
                                self.exit_market(ord_sell_tp_1, 'SELL', contracts/2, price_sell_in_1, time_sell_in, comm_sell_in_1, 'fsl1')
                                sent = False
                            ### Stop 2
                            if not self.check_pendings(ord_sell_sl_2) and self.data.iloc[-1].high - sl_sell_2 >= 1 and self.position < 0:
...
if __name__ == '__main__':
    ...
    live_nene = LiveNene(symbol=symbol, bot_name='Nene Breakout (demo)', temp='1 min', port=port, client=client, real=False)
    live_nene.run_strategy(contracts=4, stop=3, target_1=4, target_2=4, num_bars=10, trailing=4, num_days=4)
