                            ...
                            if new_bar and current_len >= kc_period+pattern_len-2:
                                ...
                                if not (self.weekday == 4 and pd.to_datetime(self.hour).time() > pd.to_datetime('12:00:00').time()):
                                    ...
                                    if not sent and sum(recognition == pattern_long) == pattern_len and prices[-2] < upper_band[-2] and prices[-1] > upper_band[-1] and plus_di[-1] > 40 and plus_di[-1] > minus_di[-1]:
                                        ...
                                        price_buy_in_1, sl_buy_1, tp_buy_1, time_buy_in, comm_buy_in_1, profit_buy, ord_buy_sl_1, ord_buy_tp_1 = self.braket_market('BUY', contracts/2, stop*brick, target_1, max_stop)
...
if __name__ == '__main__':
    ...
    live_nene = LiveNene(symbol=symbol, bot_name='Nene Breakout Longs (demo)', temp='1 min', port=port, client=client, real=False)
    live_nene.run_strategy(contracts=2, stop=3, target_1=10, target_2=3, num_bars=11, trailing=0.9, num_days=3)